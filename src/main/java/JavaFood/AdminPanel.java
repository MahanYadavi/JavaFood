...
public class AdminPanel {
    ...
    public void loadFromJSONFile(String fileAddress) {
        ...
        try (FileReader reader = new FileReader(fileAddress)) {
            ...
            for (JsonElement restaurant : restaurants) {
                ...
                Restaurant r = new Restaurant(restObj.get("id").getAsInt(), restObj.get("name").getAsString(),
                        restObj.get("address").getAsString(),
                ...
                for (JsonElement food : foods) {
                    ...
                    Food f = new Food(foodObj.get("id").getAsInt(), foodObj.get("name").getAsString(),
                            foodObj.get("price").getAsDouble());
            ...
            for (JsonElement admin : admins) {
                ...
                RestaurantOwner a = new RestaurantOwner(adminObj.get("id").getAsInt(),
                        adminObj.get("name").getAsString());
                addUser(a);
            }
            ...
            for (JsonElement customer : customers) {
                ...
                RestaurantOwner c = new RestaurantOwner(customerObj.get("id").getAsInt(),
                        customerObj.get("name").getAsString());
                addUser(c);
            }
            ...
            for (JsonElement discount : discounts) {
                ...
                if (discObj.has("amount")) {
                    Discount d = new Discount(discObj.get("id").getAsInt(), discObj.get("code").getAsString(),
                            discObj.get("amount").getAsDouble(),
                            LocalDate.parse(discObj.get("expireDate").getAsString()), discObj.get("userId").getAsInt());
                    addDiscount(d);
                } else {
                    Discount d = new Discount(discObj.get("id").getAsInt(), discObj.get("code").getAsString(),
                            discObj.get("percentage").getAsInt(),
                            LocalDate.parse(discObj.get("expireDate").getAsString()), discObj.get("userId").getAsInt());
                    addDiscount(d);
                }
            }
            ...
            for (JsonElement order : orders) {
                ...
                Order o = createOrder(orderObj.get("id").getAsInt(), orderObj.get("userId").getAsInt(),
                        orderObj.get("restaurantId").getAsInt(),
                        Order.ReceivingType.valueOf(orderObj.get("receivingType").getAsString()));
    ...
    public Order createOrder(Integer id, Integer userId, Integer restaurant_id, Order.ReceivingType type) {
        return new Order(id, userId,
                restaurants.stream().filter(r -> Objects.equals(r.getId(), restaurant_id)).findFirst().orElse(null),
                type);
    }}
